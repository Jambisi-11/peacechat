...
const ChatBody = ({ contact, onClose }) => {
  ...
  useEffect(() => {
    ...
    // Join room using contact ID
    socket.emit('joinRoom', contact._id);

    // Listen for real-time messages
    socket.on('receiveMessage', (msg) => {
      // Only show message if it's relevant to this chat
      if (
        (msg.sender === contact._id && msg.receiver === user._id) ||
        (msg.sender === user._id && msg.receiver === contact._id)
      ) {
        setMessages((prev) => [...prev, msg]);
      }
    });
